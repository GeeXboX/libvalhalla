 ...
 *
 * This implementation of SHA comes from FFmpeg/libavutil. The part related
 * to SHA-1 is removed. Only SHA-224 and SHA-256 are supported here.
 *
...
struct vh_sha_s {
  ...
  uint8_t  buffer[64]; /* 512-bit of input values used in hash updating */
  uint32_t state[8];   /* current hash value */
  /* function used to update hash for 512-bit input block */
  void     (*transform) (uint32_t *state, const uint8_t buffer[64]);
};

const int vh_sha_size = sizeof (vh_sha_t);
...
static void
sha256_transform (uint32_t *state, const uint8_t buffer[64])
{
...
int
vh_sha_init (vh_sha_t *ctx, int bits)
{
...
void
vh_sha_update (vh_sha_t *ctx, const uint8_t *data, unsigned int len)
{
  unsigned int i, j;
...
void
vh_sha_final (vh_sha_t *ctx, uint8_t *digest)
{
  int i;
